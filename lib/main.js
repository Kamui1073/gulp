const { ref, onMounted, h } = Vue
const { message } = antd
const app = Vue.createApp({
    setup() {
        const data = ref({
            //主题模式
            isDark:true,
            //弹窗
            visible:false,
            //显示关键字或分类
            isShow:true,
            //搜索关键字内容
            searchValue:'',
            //搜索关键字展示
            searchWord:'',
            //项目路径
            link:'app/',
            //标题
            htmlTitle : '文件',
            //项目数量
            number:'',
            //项目分类
            tags:'',
            tagData: _tagData,
            //分类标题图标
            titleIcon:'server',
            //分类标题颜色
            titleIconColor:'#999999',
            //总项目列表
            dataSource: [],
            columns: [],
            //默认选中分类
            dataTag: '全部',
            //说明弹窗内容
            htmlData:'',
            //表格分页
            pagination: {
                //页面位置
                current: 1,
                //设置页面
                onChange:(page,pageSize)=>{
                    data.value.pagination.current = page
                },
                pageSize: 6,
            }
        })

        const resetSearch = ()=>{
            if(data.value.searchValue !== ''){
                getList()
            }else {
                message.error('未输入文字无需重置！')
            }
        }

        //关闭弹窗
        const hideModal = ()=> {
            data.value.visible = false
        }
        //重置搜索

        const onSearch = ()=>{
            // 如果标题中包含关键字，插入到列表中
            let dataArr = []
            if(data.value.searchValue !== ''){
                _list.forEach(item => {
                    if(item.title.includes(data.value.searchValue)){
                        dataArr.push(item)
                    }
                })
                if(dataArr.length > 0){
                    //重置页码
                    data.value.pagination.current = 1
                    //分类标题图标与颜色
                    data.value.titleIcon='search'
                    data.value.titleIconColor='#ff7875'
                    //关闭分类总数显示
                    data.value.isShow = false
                    //列表置空
                    data.value.dataSource = dataArr
                    //打印关键字
                    data.value.searchWord = data.value.searchValue
                    //设置分类总数
                    console.log(data.value.dataSource.length)
                    data.value.number = data.value.dataSource.length || 0
                }else {
                    //渲染提示信息
                    const vNode = h('span', {
                        class: 'vNode'
                    },[
                        h('span','无法搜索到带有 '),
                        h('span', {style:'color:red;font-weight:bold'}, data.value.searchValue),
                        h('span', ' 关键字的内容，请重新输入！')
                    ])
                    //清空列表
                    data.value.number = 0
                    data.value.dataSource = []
                    message.error({content: vNode})
                }
            }else {
                message.error('请输入文字！')
            }
        }
        //设置分类菜单方法
        const setTagList = (e)=>{
            data.value.dataSource = []
            data.value.searchValue = ''
            pushList(data.value.dataSource,e.target.value)
            //分类
            data.value.tags = data.value.dataTag
            //分类总数
            data.value.number = data.value.dataSource.length || 0
            //console.log(data.value.dataSource)
        }
        //分类
        const handleSizeChange = (e)=> {
            //重置页码
            data.value.pagination.current = 1
            //显示分类总数
            data.value.isShow = true;
            data.value.dataTag = e.target.value
            data.value.tagData.forEach(item=> {
                if(item.name === e.target.value){
                    if(e.target.value === '全部'){
                        getList()
                    }else {
                        setTagList(e)
                    }
                    data.value.titleIcon = item.icon
                    data.value.titleIconColor = item.iconColor
                }
            })
        }
        const getList = ()=>{
            //分类标题图标与颜色
            data.value.titleIcon = 'server'
            data.value.titleIconColor = '#999999'
            //清空数据
            data.value.dataSource = []
            //加载全部分类
            data.value.dataSource = _list
            //console.log(data.value.dataSource)
            data.value.tags = ''
            data.value.searchValue = ''
            data.value.searchWord = ''
            data.value.number = data.value.dataSource.length || 0
        }
        //获取分类方法
        const pushList = (arr,tag)=>{
            _list.forEach(item => {
                if(item.tag === tag){
                    arr.push(item)
                }
            })
        }
        //高亮搜索文字方法
        const highLight = (title)=>{
            // 如果标题中包含，关键字就替换一下
            if(title.includes(data.value.searchWord)){
                title = title.replace(
                    data.value.searchWord,
                    // 这里是替换成html格式的数据
                    '<span style="color:red;">'+ data.value.searchWord +'</span>'
                )
                return title
            }
            // 不包含直接返回标题
            else{
                return title
            }
        }

        //切换主题
        const createLink = ()=>{
            // 创建一个link元素
            let link = document.createElement('link')
            // 设置link属性
            link.id = 'dark'
            link.rel = 'stylesheet' // 指定关系类型为样式表
            link.href = 'lib/antd.dark.css' // 指定样式表文件的路径
            // 将link添加到head中
            document.head.appendChild(link)
        }
        const removeLink = ()=>{
            let link = document.querySelector('#dark'); // 获取link元素
            let parent = link.parentNode; // 获取父节点
            parent.removeChild(link); // 将link元素从文档中移除
        }
        const setTheme = ()=>{
            if(document.querySelector('#dark')){
                removeLink()
                data.value.isDark = false
            }else {
                createLink()
                data.value.isDark = true
            }
        }
        onMounted(() => {
            data.value.htmlTitle = _Basic_Title
            //页面标题
            document.title = _Basic_Title
            //列表内容
            data.value.number = data.value.dataSource.length || 0
            //表头
            data.value.columns = _columns
            //console.log(data.value.columns)
            data.value.htmlData = _htmlData
            //setList(this.kjList,'科技')
            getList()
        })

        return {
            data,
            resetSearch,
            hideModal,
            onSearch,
            setTagList,
            handleSizeChange,
            getList,
            pushList,
            highLight,
            setTheme
        }
    }
})
app.use(antd)
app.mount('#app')
